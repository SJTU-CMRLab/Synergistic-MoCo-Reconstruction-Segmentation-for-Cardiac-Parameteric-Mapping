# %%
# Extract the pretrained PE/SS .rar archives in place before running this script.
import torch
...
if __name__ == '__main__':
    ...
    for epoch_ind in range(epoch_num):
        ...
        for train_xb_im, train_xb_RR, train_xb_true_mask, train_xb_frame_order in train_dl:

            train_xb_im, train_xb_RR, train_xb_true_mask = (
                train_xb_im.to(device), train_xb_RR.to(device), train_xb_true_mask.to(device))

            # Registration
            train_xb_def_im, train_xb_vel, _, train_xb_inv_disp = reg_net(train_xb_im)

            # Image synthesis
            train_xb_map, train_xb_syn_im = image_synthesis(train_xb_def_im, train_xb_RR, 'eval')
            ...
            if iter_ind % 100 == 0:
                ...
                with torch.no_grad():
                    ...
                    valid_xb_Lr_record = []
                    valid_xb_Ls_record = []
                    for valid_xb_im, valid_xb_RR, valid_xb_true_mask, valid_xb_frame_order in valid_dl:

                        valid_xb_im, valid_xb_RR, valid_xb_true_mask = (
                            valid_xb_im.to(device), valid_xb_RR.to(device), valid_xb_true_mask.to(device))

                        # Registration
                        valid_xb_def_im, valid_xb_vel, _, valid_xb_inv_disp = reg_net(valid_xb_im)

                        # Image synthesis
                        valid_xb_map, valid_xb_syn_im = image_synthesis(valid_xb_def_im, valid_xb_RR, 'eval')
