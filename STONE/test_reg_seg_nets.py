...
if __name__ == '__main__':
    ...
    with torch.no_grad():
        ...
        test_xb_Lr_record = []
        test_xb_Ls_record = []
        for test_xb_im, test_xb_TI, test_xb_true_mask, _ in test_dl:

            test_xb_im, test_xb_TI, test_xb_true_mask = (
                test_xb_im.to(device), test_xb_TI.to(device), test_xb_true_mask.to(device))

            # Registration
            test_xb_def_im, test_xb_vel, _, test_xb_inv_disp = reg_net(test_xb_im)

            # Image synthesis
            test_xb_map, test_xb_syn_im = image_synthesis(test_xb_def_im, test_xb_TI, 'eval')
...
if __name__ == '__main__':
    sub_ind = 1
    slice_ind = 1
    im, TI = RegDataset(data_dir, [sub_ind], [slice_ind], ds_transforms=transforms.CenterCrop(size=im_shape))[0]
    im, TI = map(lambda x: torch.unsqueeze(x, dim=0).to(device), [im, TI])

    time_scaling = 1000
    with torch.no_grad():
        pmap = parameter_estimation(im, TI) * time_scaling

        new_im, _, disp, inv_disp = reg_net(im)
        new_pmap = parameter_estimation(new_im, TI)
    ...
    fig, axs = plt.subplots(2, 11, figsize=(22, 4))
    ...
    tmp = axs[0].imshow(pmap[0, 0, :, :].cpu(), cmap='jet', vmin=0, vmax=2200)
    fig.colorbar(tmp, ax=axs[0], shrink=0.9, pad=0.05)
    axs[0].axis('off')
    axs[0].set_title('original T1')

    tmp = axs[1].imshow(new_pmap[0, 0, :, :].cpu(), cmap='jet', vmin=0, vmax=2200)
    fig.colorbar(tmp, ax=axs[1], shrink=0.9, pad=0.05)
    axs[1].axis('off')
    axs[1].set_title('corrected T1')

    tmp = axs[2].imshow(new_pmap[0, 0, :, :].cpu(), cmap='jet', vmin=0, vmax=2200)
    fig.colorbar(tmp, ax=axs[2], shrink=0.9, pad=0.05)
    axs[2].imshow(one_im, cmap='Blues', vmin=0, vmax=1, alpha=alpha_mask)
    axs[2].axis('off')
    axs[2].set_title('segmented T1')

    tmp = axs[3].imshow(pred_mask[0, 0, :, :].cpu(), cmap='gray', vmin=0, vmax=2)
    fig.colorbar(tmp, ax=axs[3], shrink=0.9, pad=0.05)
    axs[3].axis('off')
...
if __name__ == '__main__':
    ...
    slice_ind_record = list(range(1, 6))

    run_time_list = []
    for sub_ind in sub_ind_record:
        for slice_ind in slice_ind_record:

            im, TI, true_mask, _ = RegSegDataset(data_dir, [sub_ind], [slice_ind], im_shape=im_shape,
                                                 ds_transforms1=None, ds_transforms2=None)[0]
            im, TI, true_mask = map(lambda x: torch.unsqueeze(x, dim=0).to(device), [im, TI, true_mask])
            ...
            with torch.no_grad():
                ...
                pmap, syn_im = image_synthesis(im, TI, 'eval')
                ...
                new_pmap, new_syn_im = image_synthesis(new_im, TI, 'eval')
                tmp = torch.concatenate([new_pmap, new_im], dim=1)
                _, pred_mask = seg_net(tmp)
                end.record()
            ...
            pmap = pmap[0, 0, :, :].cpu().numpy()
            new_pmap = new_pmap[0, 0, :, :].cpu().numpy()
            ...
            save_dict = {'TI': TI,
                         'im': im, 'new_im': new_im,
                         'T1_map': pmap, 'new_T1_map': new_pmap,
